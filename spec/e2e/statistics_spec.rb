...
RSpec.describe 'Statistics', type: :aruba do
  ...
  # <REQ> The software shall provide the "Number of Items w/ Test Coverage" for each specification >[SRS-008] </REQ>
  describe 'Number of Items with Test Coverage' do
    let(:idx) { Nokogiri::HTML(File.read(expand_path('myproject/build/index.html'))) }

    it 'renders 1 covered item for REQ' do
      cell = idx.at_xpath('(//tr[.//a[contains(@href,"specifications/req/req.html")]]//td[@class="item_id"])[4]')
      expect(cell).not_to be_nil
      expect(cell.text.strip).to eq('1')
    end

    it 'renders 0 covered items for ARCH' do
      cell = idx.at_xpath('(//tr[.//a[contains(@href,"specifications/arch/arch.html")]]//td[@class="item_id"])[4]')
      expect(cell).not_to be_nil
      expect(cell.text.strip).to eq('0')
    end
  end
